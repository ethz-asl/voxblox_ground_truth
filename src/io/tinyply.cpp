...
struct PlyFile::PlyFileImpl {
  ...
  size_t read_property_binary(const Type &t, const size_t &stride, void *dest,
  ...
  void write_property_ascii(Type t, std::ostream &os, uint8_t *src,
                            size_t &srcOffset);
  void write_property_binary(Type t, std::ostream &os, uint8_t *src,
...
void PlyFile::PlyFileImpl::read_header_text(std::string line, std::istream &is,
...
size_t PlyFile::PlyFileImpl::read_property_binary(const Type &t,
                                                  const size_t &stride,
                                                  void *dest,
                                                  size_t &destOffset,
                                                  std::istream &is) {
...
void PlyFile::PlyFileImpl::write_property_binary(Type t, std::ostream &os,
                                                 uint8_t *src,
                                                 size_t &srcOffset,
                                                 const size_t &stride) {
  ...
  for (auto &b : buffers) {
    for (auto &entry : userData) {
      if (entry.second.data == b && b->buffer.get() == nullptr) {
        ...
        // the
        // file to compute the total length of all (potentially) variable-length
        // lists
        if (list_hints == 0) {
    ...
    for (size_t i = 0; i < e.size; ++i) {
      for (auto &p : e.properties) {
        ...
        if (p.isList) {
          ...
          write_property_ascii(p.propertyType, os, (helper.data->buffer.get() +
                                                    helper.cursor->byteOffset),
                               helper.cursor->byteOffset);
        }
      }
...
std::shared_ptr<PlyData> PlyFile::PlyFileImpl::request_properties_from_element(
  ...
  // hash of
  // element name and property name), but groups of properties (requested from
  // the
  // public api through this function) all share the same `ParsingHelper`. When
  // it comes
      ...
      if (propertyIndex >= 0) {
        ...
        if (result.second == false) {
          throw std::invalid_argument(
              ...
              hash_fnv1a(element.name + property.name));
        }
      } else
...
void PlyFile::PlyFileImpl::parse_data(std::istream &is, bool firstPass) {
  ...
  auto read_list_binary = [this](const Type &t, void *dst, size_t &destOffset,
                                 std::istream &_is) {
    const size_t stride =
        PropertyTable[t].stride;  // @todo - this is already precomputed
    destOffset += stride;
  ...
  if (isBinary) {
    read = [this, &listSize, &dummyCount, &read_list_binary](
      ...
      if (!p.isList) {
        read_property_binary(p.propertyType, f.prop_stride, dest + destOffset,
                             destOffset, _is);
      } else {
    ...
    skip = [this, &listSize, &dummyCount, &read_list_binary](
        PropertyLookup &f, const PlyProperty &p, std::istream &_is) {
      if (!p.isList) {
      ...
      return read_property_binary(p.propertyType, f.prop_stride * listSize,
                                  scratch, dummyCount, _is);
    };
  } else {
      ...
      for (auto &property : element.properties) {
        ...
        if (!f.skip) {
          ...
          if (firstPass)
            ...
            read(f, property, helper->data->buffer.get(),
